...
bool GetBindHash(uint160& hash, CTxOut const& txout) {
    ...
    if (0 <= opcode && opcode <= OP_PUSHDATA4) {
        //one OP_IF followed by sender id_hash
        ...
        //two OP_IFs followed by delegate id_hash
        if (OP_IF != opcode) {
            return false;
        }
        if (position >= payload.end()) {
...
bool GetBindHash(uint160& hash, CTransaction const& tx) {
    for (
        std::vector<CTxOut>::const_iterator txout = tx.vout.begin();
        tx.vout.end() != txout;
        txout++
    ) {
        if (GetBindHash(hash, *txout)) {
            return true;
        }
    }
    return false;
...
void InitializeBind(
    ...
    CTransaction const rawTx = CreateBind(
            local,
            address_bind_nonce,
            nAmount,
            delegateFee,
    ...
    if (!GetBindHash(id_hash, rawTx)) {
        throw std::runtime_error("failure creating transaction");
    }

    pwalletMain->store_hash_delegate( id_hash, my_key);

    if (is_delegate) {
        //store recovery address for retrieval
        uint64_t address_bind_nonce;

        if(!pwalletMain->GetBoundNonce(other, address_bind_nonce)) {
           printf("InitializeDelegateBind() : could not find nonce for address %s \n",
                  other.ToStringIP().c_str());
        } else {
            pwalletMain->add_to_retrieval_string_in_nonce_map(address_bind_nonce, recovery_address.ToString(), true);
            printf("InitializeDelegateBind() : wrote recovery address to retrieve string %s \n", recovery_address.ToString().c_str());
        }
    } else {
        pwalletMain->add_to_retrieval_string_in_nonce_map(address_bind_nonce, recovery_address.ToString(), false);
