...
Value createtransferexpiry(const Array& params, bool fHelp)
{
    ...
    uint256 const bind_tx_id = ParseHashV(params[0], "parameter 1");
    ...
    CTransaction prevTx;
    uint256 hashBlock = 0;
    if (!GetTransaction(bind_tx_id, prevTx, hashBlock)) {
    ...
    for (
        ...
        if (
            ...
            CTxIn claiming;
            claiming.prevout = COutPoint(bind_tx_id, output_index);
